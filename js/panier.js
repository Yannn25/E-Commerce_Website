...
function setCookie(cname, cvalue, exhours) {
    let d = new Date();
    d.setTime(d.getTime() + (exhours * 60 * 60 * 1000)); // Conversion des heures en millisecondes
    let expires = "expires=" + d.toUTCString();
  
    // Résout le problème des caractères interdits
    if ('btoa' in window) {
  ...
  function saveCart(inCartItemsNum, cartArticles) {
    setCookie('inCartItemsNum', inCartItemsNum, 4); // Durée de validité de 4 heures
    setCookie('cartArticles', JSON.stringify(cartArticles), 4); // Durée de validité de 4 heures
  }
...
$('.panier_button').click(function() {
    ...
    let qt = 1;
    let taille = $(this).closest('.card').find('select').val();
...
if (window.location.pathname == '/panier/') {
    ...
    */
    ...
    $('.qt-plus').on('click', function() {
        ...
        let artWeight = parseInt($this.parent().parent().attr('data-weight'));
